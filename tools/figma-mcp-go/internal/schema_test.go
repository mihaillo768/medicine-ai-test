...
func TestValidateRPC_CreateComponentInstance(t *testing.T) {
	if msg := ValidateRPC("create_component_instance", nil, map[string]interface{}{}); msg == "" {
		t.Error("expected error when componentId missing")
	}
	if msg := ValidateRPC("create_component_instance", nil, map[string]interface{}{
		"componentId": "1:1",
	}); msg == "" {
		t.Error("expected error when parentId missing")
	}
	if msg := ValidateRPC("create_component_instance", nil, map[string]interface{}{
		"componentId": "bad",
		"parentId":    "2:2",
	}); msg == "" {
		t.Error("expected error for bad componentId")
	}
	msg := ValidateRPC("create_component_instance", nil, map[string]interface{}{
		"componentId": "1:1",
		"parentId":    "2:2",
		"replaceNodeId": "3-3",
	})
	if msg == "" {
		t.Error("expected error for bad replaceNodeId")
	}
	msg = ValidateRPC("create_component_instance", nil, map[string]interface{}{
		"componentId": "1:1",
		"parentId":    "2:2",
