...
func ValidateRPC(tool string, nodeIDs []string, params map[string]interface{}) string {
	...
	case "resize_nodes":
		...
		tw, _ := params["textWrap"].(bool)
		tfb, _ := params["textFixedBox"].(bool)
		if tw && tfb {
			return "use only one of textWrap or textFixedBox"
		}
		if tw && !hasW {
			return "textWrap requires width for TEXT nodes"
		}
		if tfb && (!hasW || !hasH) {
			return "textFixedBox requires both width and height for TEXT fixed layout"
		}
		if !hasW && !hasH {
	...
	case "create_component_instance":
		if cid, _ := params["componentId"].(string); cid == "" {
			return "componentId is required"
		} else if !ValidNodeID(cid) {
			return fmt.Sprintf("componentId must use colon format e.g. 4029:12345, got: %s", cid)
		}
		if pid, _ := params["parentId"].(string); pid == "" {
			return "parentId is required"
		} else if !ValidNodeID(pid) {
			return fmt.Sprintf("parentId must use colon format e.g. 4029:12345, got: %s", pid)
		}
		if rid, ok := params["replaceNodeId"].(string); ok && rid != "" && !ValidNodeID(rid) {
			return fmt.Sprintf("replaceNodeId must use colon format e.g. 4029:12345, got: %s", rid)
		}
