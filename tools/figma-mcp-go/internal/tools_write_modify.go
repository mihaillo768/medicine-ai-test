...
func registerWriteModifyTools(s *server.MCPServer, node *Node) {
	...
	s.AddTool(mcp.NewTool("resize_nodes",
		mcp.WithDescription("Resize one or more nodes. The same width/height is applied to every node in the list independently. Provide width, height, or both. For TEXT: set textWrap=true and width to enable line wrapping (HEIGHT auto-resize); set textSizingReset=true before width/height to restore single-line WIDTH_AND_HEIGHT sizing (e.g. revert)."),
		mcp.WithArray("nodeIds",
			mcp.Required(),
			mcp.Description("Node IDs in colon format e.g. ['4029:12345']"),
			mcp.WithStringItems(),
		...
		mcp.WithBoolean("textWrap", mcp.Description("If true and node is TEXT: set textAutoResize HEIGHT and fixed width (requires width); height fits wrapped text.")),
		mcp.WithBoolean("textFixedBox", mcp.Description("If true and node is TEXT: set textAutoResize NONE and resize to exact width×height (fixed box, text wraps inside). Requires both width and height.")),
		mcp.WithBoolean("textSizingReset", mcp.Description("If true and node is TEXT: set textAutoResize WIDTH_AND_HEIGHT before applying width/height (single-line box).")),
	), func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		raw, _ := req.GetArguments()["nodeIds"].([]interface{})
		nodeIDs := toStringSlice(raw)
		params := map[string]interface{}{}
		...
		if tw, ok := req.GetArguments()["textWrap"].(bool); ok {
			params["textWrap"] = tw
		}
		if tfb, ok := req.GetArguments()["textFixedBox"].(bool); ok {
			params["textFixedBox"] = tfb
		}
		if ts, ok := req.GetArguments()["textSizingReset"].(bool); ok {
			params["textSizingReset"] = ts
		}
