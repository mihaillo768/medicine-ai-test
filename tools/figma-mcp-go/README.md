...
> **Форк в этом репозитории** (`tools/figma-mcp-go`): добавлен инструмент MCP **`create_component_instance`** — создание `INSTANCE` из мастер-`COMPONENT` внутри родителя с опциональной заменой плейсхолдера (см. `FORK-NOTES.md` в этой папке). Остальное совпадает с [upstream](https://github.com/vkhanhqui/figma-mcp-go).
...
| `create_component_instance` | Create a new INSTANCE of a COMPONENT under a parent; optional `replaceNodeId` removes a placeholder at the same index |
